...
```{r}
A = matrix(as.numeric(str_split("1,0,1,0", pattern = ",")[[1]]), 2, 2, byrow = T)
    a = matrix(as.numeric(str_split("1,0", pattern = ",")[[1]]), 2)
    a0 = as.numeric("0")
   ...
   variance_is <- IS(a0, a, A, Sigma, x_var)
   variance_is <-  c(round(variance_is[1],4), round(variance_general[2]/variance_is[2]),0)
   
   variance_cv <- CV(a0, a, A, Sigma, x_var)
   variance_cv <-  c(round(variance_cv[1],4), round(variance_general[2]/variance_cv[2]),0)
   
   variance_ss <- SS(a0, a, A, Sigma, x_var)
   variance_ss <-  c(round(variance_ss[1],4), round(variance_general[2]/variance_ss[2]),0)
